    ...
    emptySequences.some(s => {
      for (let x = s.start; x <= s.end - window.width + 1; x += 1) {
        ...
        if (verticalSizeOfWindowIsFree(representation, {y, x}, window)) {
          // TODO: move this logic to occupy-window, make it
          // first occupy outer frame of window, then straight and diagnoal cross
          // that way, no need to check before
          try {
            occupy(representation, Object.assign({}, window, {x, y}))
            occupied = {x, y}
            return true
          } catch (e) {
            return false
